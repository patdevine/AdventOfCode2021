package adventofcode2021.day.eight;
...
public class SevenSegmentSearchPart2 {
    ...
    enum Digit {
        ...
        public static Digit fromCount(String value) {
            int valueCharCount = value.length();
            switch (valueCharCount) {
                case 2:
                    return ONE;
                case 3:
                    return SEVEN;
    ...
    public static int displayCount(String input) {
        String swappedInput = input;
        String[] displaySplit = input.split("\\s\\|\\s");

        String[] signalPatterns = displaySplit[0].split(" ");
        String[] outputValues = displaySplit[1].split(" ");

        Map<Digit, String> mapping = new HashMap<>();

        for (String signal : signalPatterns) {
            Digit digit = Digit.fromCount(signal);
            if (Digit.ONE == digit) {
                mapping.putIfAbsent(Digit.ONE, signal);
            } else if (Digit.SEVEN == digit) {
                mapping.putIfAbsent(Digit.SEVEN, signal);
            } else if (Digit.FOUR == digit) {
                mapping.putIfAbsent(Digit.FOUR, signal);
            } else if (Digit.EIGHT == digit) {
                mapping.putIfAbsent(Digit.EIGHT, signal);
            }
        }


        String removeOneFromFour = mapping.get(Digit.FOUR).replaceAll(mapping.get(Digit.ONE).substring(0, 1), "").replaceAll(mapping.get(Digit.ONE).substring(1, 2), "");

        for (String signal : signalPatterns) {
            //map 2,3,5
            if (signal.length() == 5) {
                if (signal.indexOf(mapping.get(Digit.ONE).charAt(0)) != -1 && signal.indexOf(mapping.get(Digit.ONE).charAt(1)) != -1) {
                    mapping.put(Digit.THREE, signal);
                } else if (signal.indexOf(removeOneFromFour.charAt(0)) != -1 && signal.indexOf(removeOneFromFour.charAt(1)) != -1) {
                    mapping.put(Digit.FIVE, signal);
                } else {
                    mapping.put(Digit.TWO, signal);
                }
                //map 6,9,0
            } else if (signal.length() == 6) {
                if (!(signal.indexOf(mapping.get(Digit.ONE).charAt(0)) != -1 && signal.indexOf(mapping.get(Digit.ONE).charAt(1)) != -1)) {
                    mapping.put(Digit.SIX, signal);
                } else if (signal.indexOf(removeOneFromFour.charAt(0)) != -1 && signal.indexOf(removeOneFromFour.charAt(1)) != -1) {
                    mapping.put(Digit.NINE, signal);
                } else {
                    mapping.put(Digit.ZERO, signal);
                }
            }
        }
        ...
        for (String outputValue : outputValues) {
            for (Digit digit : Digit.values()) {
                String signal = mapping.get(digit);
                if (containsAllSameCharactersAndSameLength(signal, outputValue)) {
                    stringBuffer.append(digit.value);
                }
            }
        }
        ...
        int output = Integer.valueOf(stringBuffer.toString());

        System.out.println("output: " + output);
        return output;
    }

    public static boolean containsAllSameCharactersAndSameLength(String one, String two) {
        if ( one == null || two == null || one.length() != two.length()) return false;

        for (char aChar : one.toCharArray()) {
            if (!two.contains(String.valueOf(aChar))) {
                return false;
            }
        }
        return true;
    ...
    public static void main(String[] args) {
        int totalCount = Arrays.stream(input.split("\n")).map(SevenSegmentSearchPart2::displayCount).mapToInt(Integer::intValue).reduce(0, (count, current) -> count + current);
        System.out.println("count is " + totalCount);
    }

    public static final String example2 = "acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf";
